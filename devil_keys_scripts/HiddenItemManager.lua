...
-- Modified by Thicco Catto to spawn regular wisps instead of item wisps
-- Version 2.0
--
...
local function GetWisp(tab)
    ...
    if ptr and ptr.Ref then
        if ptr.Ref.Type == EntityType.ENTITY_FAMILIAR and ptr.Ref.Variant == FamiliarVariant.WISP then
...
local function KillWisp(wisp)
    ...
    elseif wisp.Type == EntityType.ENTITY_FAMILIAR and wisp.Variant == FamiliarVariant.WISP then
...
local function RemoveWisp(key)
    local tab = HiddenItemManager.Mod.Data.INDEX[key]
    if not tab then return end
    local player = GetPlayer(tab)
    local subtype = tab.Subtype

    if player then
        ClearData(GetPlayerKey(player), tab.Group, subtype, key)
    else
        ...
        for playerKey, playerData in pairs(DATA) do
            ClearData(playerKey, tab.Group, subtype, key)
        end
    end

    HiddenItemManager.Mod.Data.INDEX[key] = nil
end
...
local function InitializeWisp(wisp)
    ...
    local tab = HiddenItemManager.Mod.Data.INDEX[wispKey]
    tab.WispKey = wispKey
...
---@param duration any
---@param group any
---@param removeOnNewRoom any
---@param removeOnNewLevel any
local function SpawnWisp(player, itemID, duration, group, removeOnNewRoom, removeOnNewLevel)
    ...
    local wisp = player:AddWisp(itemID, kWispPos)
    local wispKey = GetWispKey(wisp)
    local tab = {
        Subtype = wisp.SubType,
        Group = group,
        Duration = duration,
    ...
    HiddenItemManager.Mod.Data.INDEX[wispKey] = tab
    InitializeWisp(wisp)
...
function HiddenItemManager:LoadData()
    if not HiddenItemManager.Mod.Data.INDEX then
        HiddenItemManager.Mod.Data.INDEX = {}
    end

    for _, data in pairs(HiddenItemManager.Mod.Data.INDEX) do
...
function HiddenItemManager:ItemWispUpdate(wisp)
    ...
    local wispKey = GetWispKey(wisp)
    local wispData = HiddenItemManager.Mod.Data.INDEX[wispKey]
    ...
    if wispData then
        ...
        if not IsManagedWisp(wisp) then
            ...
            InsertData(playerKey, wispData.Group, wispData.Subtype, wispKey, wispData)
            InitializeWisp(wisp)
        end
...
AddCallback(ModCallbacks.MC_FAMILIAR_INIT, HiddenItemManager.ItemWispUpdate, FamiliarVariant.WISP)
AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, HiddenItemManager.ItemWispUpdate, FamiliarVariant.WISP)
...
AddLateCallback(ModCallbacks.MC_FAMILIAR_UPDATE, HiddenItemManager.ItemWispLateUpdate, FamiliarVariant.WISP)
...
function HiddenItemManager:PostUpdate()
    ...
    for key, data in pairs(HiddenItemManager.Mod.Data.INDEX) do
        ...
        if data.Initialized and not wisp and IsActivePlayer(player) then
            if data.ErrorCount >= 10 then
                ...
                data.Subtype .. " from group: " .. data.Group)
                RemoveWisp(key)
            else
    ...
    for oldKey, data in pairs(wispsToRespawn) do
        ...
        if player then
            RemoveWisp(oldKey)
            local wisp = player:AddWisp(data.Subtype, kWispPos)
            local newKey = GetWispKey(wisp)
            if newKey then
                InsertData(GetPlayerKey(player), data.Group, data.Subtype, newKey, data)
                HiddenItemManager.Mod.Data.INDEX[newKey] = data
                InitializeWisp(wisp)
            end
        end
    end
...
AddCallback(ModCallbacks.MC_PRE_FAMILIAR_COLLISION, HiddenItemManager.ItemWispCollision, FamiliarVariant.WISP)
...
function HiddenItemManager:ItemWispDamage(entity, damage, damageFlags, damageSourceRef, damageCountdown)
    if entity and entity.Type == EntityType.ENTITY_FAMILIAR and entity.Variant == FamiliarVariant.WISP and IsManagedWisp(entity) then
        return false
    end

    if damageSourceRef.Type == EntityType.ENTITY_FAMILIAR and damageSourceRef.Variant == FamiliarVariant.WISP
...
function HiddenItemManager:ItemWispTears(tear)
    if tear.SpawnerEntity and tear.SpawnerEntity.Type == EntityType.ENTITY_FAMILIAR
        and tear.SpawnerEntity.Variant == FamiliarVariant.WISP
